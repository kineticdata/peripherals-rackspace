...
class RackspaceServerDeleteV1
  ...
  def execute()
    ...
    server = service.servers.find {|i| i.id =~ Regexp.new(@parameters['server_id'])}

    if server.nil?
      raise StandardError, "Invalid Server Id: Cannot find a server with the id of #{@parameters['server_id']}"
    end
